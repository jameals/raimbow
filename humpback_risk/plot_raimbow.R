...
#------------------------------------------------------------------------------
plot_raimbow.data.frame <- function(x, x.col, grid.sf, map.base, map.i050, map.i100, map.i200, 
                                    col.breaks, col.pal, 
                                    key.pos, reset, ...) {
  ...
  plot_raimbow(tmp, x.col, NULL, map.base, map.i050, map.i100, map.i200, col.breaks, col.pal, 
               key.pos, reset, ...)
}


#------------------------------------------------------------------------------
plot_raimbow.sf <- function(x, x.col, grid.sf, map.base, map.i050, map.i100, map.i200, 
  ...
  if (!is.null(map.i050)) plot(map.i050, add = TRUE, col = "red", lwd = 0.5)
  if (!is.null(map.i100)) plot(map.i100, add = TRUE, col = "grey", lwd = 0.5)
  if (!is.null(map.i200)) plot(map.i200, add = TRUE, col = "black", lwd = 0.5)
  
  abline(h = 42, col = "black")
  abline(h = 38.833, col = "black")
  abline(h = 34.5, col = "black")
  graphics::box()
...
legend.raimbow.bathy <- function(...) {
  legend(
    "right", title = "Isobaths", legend = c("50m", "100m", "200m"), 
    col = c("red", "grey", "black"), 
    pch = NA, lty = 1, lwd = 2, cex = 1, 
    ...
  )
}
