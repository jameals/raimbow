...
README: This document is the same as 'Whale_risk_timeseries.Rmd', except using individual [zones](https://wildlife.ca.gov/Conservation/Marine/Whale-Safe-Fisheries) for regions and removing the anomaly section.

This document summarizes and plots time series of humpback whale risk of entanglement, as well as the humpback predictions and fishing measure values. It loads values generated in 'Whale_risk.Rmd', and sums them by region. These summed values, as well as their densities, are then plotted. 

The whale risk file, e.g. 'Whale_risk_long_nona.Rdata', is created in 'Whale_risk.Rmd'. See that files for details about which fishing data are used. 

This file uses 'Grid_zones.Rdata' for the regions by which to plot the data.
...
if (user == "JS") {
  ...
  file.grid.region <- "C:/SMW/RAIMBOW/raimbow-local/RDATA_files/Grid_zones.Rdata"
  
  path.plots <- "C:/SMW/RAIMBOW/raimbow-local/Plots/Whale_risk_timeseries_zones/"
  file.out.timeseries <- "C:/SMW/RAIMBOW/raimbow-local/RDATA_files/Whale_risk_timeseries_zones.Rdata"
...
grid.region.sf <- grid.zone %>% 
  mutate(region = factor(region, levels = zone.names)) %>% 
  left_join(grid.5km.lno, by = "GRID5KM_ID") %>% 
  st_sf()
plot(grid.region.sf["region"], main = "Region (RAMP zone)", 
     axes = TRUE, border = NA, pal = rev(RColorBrewer::brewer.pal(8, "Set3")), 
...
all.df <- all.df %>% 
  left_join(grid.zone, by = "GRID5KM_ID") %>% 
...
if (flag.save) 
  ggsave(filename = "Zone_linear_humpback_risk_regional.png", p123, path = path.plots, 
...
if (flag.save)
  ggsave(filename = "Zone_linear_humpback_risk_regional_areanorm.png", p123, path = path.plots, 
         height = fig.h, width = 10)
```
...
if (flag.save) 
  ggsave(filename = "Zone_linear_humpback_risk_regional_monthlydens.png", p123, path = path.plots, 
         height = fig.h, width = 10)
```
