...
if (user == "JS") {
  ...
  path.plots1 <- "../raimbow-local/Plots/Entanglements_risk/Entanglements_timeseries_v2/"
  path.plots2 <- "../raimbow-local/Plots/Entanglements_risk/Entanglements_county_timeseries_v2/"
...
y <- x %>% 
  ...
  full_join(all.df.summ.join, by = c("ym", "region")) %>%
  mutate(e_logical = factor(!is.na(CaseID)), 
         ym = factor(ym), 
...
y %>% 
  ...
  facet_wrap(facets = vars(region), nrow = facet.nrow) +
  guides(fill = guide_legend(title = "Entanglement")) + 
  xlab("Risk") + 
  ggtitle("Regional (monthly) risk for each entanglement report - original (actual) report timing")
...
for (lookback.mons in 0:12) {
  ...
  y2 <- x2 %>% 
    ...
    full_join(all.df.summ.join, by = c("ym", "region")) %>%
    mutate(e_logical = factor(!is.na(ent_idx)), 
           ym = factor(ym), 
           region = factor(region, levels(x$region))) %>% 
    arrange(region, ym)
  ...
  gg2 <- y2 %>% 
    ...
    scale_fill_discrete(drop = FALSE, name = "Entanglement") + 
    scale_y_continuous(breaks = seq(0, 100, by = 5)) +
    xlab("Risk") + 
    ggtitle(paste("Regional (monthly) risk for each entanglement report -", lookback.mons, 
