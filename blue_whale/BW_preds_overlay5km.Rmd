...
bm.sf <- st_as_sf(bm.stars) %>% #~20s
  purrr::set_names(gsub("[.]", "_", substr(names(.), 1, 13))) %>% #the naming convention may have changed between the 2009-2019 bw output, and 2019-2021 bw output. This line causes column names to lose the date component, therefore just changed the 13 to be 27 to keep all characters
  st_set_crs(4326)
```
...
## Important note: overlay_sdm() doesn't work in R version > 4? (not in 4.1.1)
