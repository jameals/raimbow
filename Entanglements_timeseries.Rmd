...
all.df.summ.join <- all.df.summ %>% 
  mutate(ym = sub("-", "_", ym), 
         region = factor(region, levels = levels(x$region))) %>% 
...
ggsave("../raimbow-local/Plots/Entanglementreport_timeseries/Risk_hist_orig.png", 
       height = 8, width = 14)
```
...
for (lookback.mons in 0:12) {
  ...
  x2 <- lapply(0:lookback.mons, function(i) {
    x.pre %>% 
      mutate(ym_date_curr = ym_date %m-% months(i), 
             ym_curr = sub("-", "_", substr(as.character(ym_date_curr), 1, 7)))
  }) %>% 
    bind_rows() %>% 
  ...
  # Save plot
  plot.path <- paste0(
    "../raimbow-local/Plots/Entanglementreport_timeseries/Risk_hist_lookback_", 
    lookback.mons, "mons.png"
  )
  ggsave(plot.path, gg2, height = 8, width = 14)
}
```
