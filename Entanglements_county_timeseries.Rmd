...
README: This document is the same as 'Entanglements_timeseries.Rmd', except using counties for regions.
...
if (user == "JS") {
  ...
  path.plots <- "../raimbow-local/Plots/Entanglements_county_timeseries/"
...
county.levels <- levels(all.df.summ$region)[!(levels(all.df.summ$region) %in% county.no)]

x <- x.orig %>% 
  select(CaseID, Year, Month, Report_Lat, Report_Long, Report_County, 
         Gear_Set_County, Gear_Set_Location_State, Timing) %>% 
  mutate(region = gsub(" ", "_", Gear_Set_County)) %>% 
  # left_join(county.reg, by = "county") %>% 
...
ggplot(x, aes(x = ym, fill = region)) + 
  ...
  scale_fill_discrete(name = "region", drop = FALSE) + 
...
ggplot(all.df.summ, aes(x = risk_sum_dens)) +
  ...
  facet_wrap(facets = vars(region), nrow = 3) +
  xlab("Risk") + 
  ggtitle("Risk time series")

ggsave(paste0(path.plots, "Risk_hist_all.png"), height = 12, width = 14)


# Plot histogram of time series risk values, color-coded by whether or not there was an entanglement
y %>% 
  filter(!(region %in% c("WA", "OR"))) %>%
  ggplot(aes(x = risk_sum_dens, fill = e_count_fac)) + 
  geom_histogram(binwidth = hist.width, position = "stack", boundary = 0) +
  # facet_grid(cols = vars(region)) +
  facet_wrap(facets = vars(region), nrow = 3) +
...
ggsave(paste0(path.plots, "Risk_hist_orig.png"), height = 12, width = 14)
```
...
for (lookback.mons in 0:12) {
  ...
  y2 <- x2 %>% 
    ...
    right_join(all.df.summ.join, by = c("ym", "region")) %>%
    replace_na(list(e_count = 0)) %>%
    mutate(e_count_fac = factor(e_count, levels = 0:5), 
           e_count_any = e_count > 0, 
           ym = factor(ym), 
           region = factor(region, levels(x$region)))
  ...
  # Plot
  gg2 <- y2 %>% 
    filter(region %in% sort(unique(as.character(x2$region)))) %>%
    ggplot(aes(x = risk_sum_dens, fill = e_count_fac)) + 
    geom_histogram(binwidth = hist.width, position = "stack", boundary = 0) +
  ...
  ggsave(file.plot, gg2, height = 12, width = 14)
...
table(x.out$ym_max0, as.character(x.out$region.x))
table(x.out$ym_max3, as.character(x.out$region.x))
table(x.out$ym_max6, as.character(x.out$region.x))
table(x.out$ym_max9, as.character(x.out$region.x))
table(x.out$ym_max12, as.character(x.out$region.x))
```
