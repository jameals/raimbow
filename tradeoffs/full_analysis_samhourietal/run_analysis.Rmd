...
annual_df_ranks <- annual_statewide_df %>%
  group_by(crab_year) %>%
  mutate(
    # if revenue changes by <=1%, set relative_dollars_cost equal to 1
    relative_dollars_cost = ifelse(relative_dollars >=99, 1 , 100 - relative_dollars),
    ...
    # if risk goes up, set benefit-cost ratio equal to zero
    # if revenue changes by <=1%, set relative_dollars_cost equal to 1
...
# make some histograms by depth. added 020121 from "evaluate fishing depths.R"
...
quantile(tmp$NGDC_M/10, prob=0.39)
