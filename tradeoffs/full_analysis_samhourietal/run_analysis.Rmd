...
Remember to modify make tradeoffs function to write our all vessels rds.
...
annual_df_ranks <- annual_statewide_df %>%
  ...
  mutate(
    ...
    benefits_costs_hump = ifelse(relative_hump_risk_n >=0, relative_hump_risk_n/relative_dollars_cost, 0),
    benefits_costs_blwh = ifelse(relative_blwh_risk_n >=0, relative_blwh_risk_n/relative_dollars_cost, 0),
    ...
    # 
    rank_bchump = row_number(benefits_costs_hump), 
    rank_bcblwh = row_number(benefits_costs_blwh) 
  ...
  select(crab_year, plotting_year, pre_post, 
         ...
         #rank_cbhump,rank_cbblwh,
         rank_bchump, rank_bcblwh,
         ...
         benefits_costs_hump, benefits_costs_blwh
  )
...
annual_df_ranks_period <- annual_statewide_df %>%
  ...
  summarise(
    ...
    mean_bchump = mean(benefits_costs_hump, na.rm=TRUE),
    mean_bcblwh = mean(benefits_costs_blwh, na.rm=TRUE),
    .groups = "drop"
  ) %>%
  group_by(pre_post) %>%
  mutate(
    ...
    rank_bchump = row_number(mean_bchump),
    rank_bcblwh = row_number(mean_bcblwh),
    .groups = "drop"
  ) %>%
  select(pre_post, 
         scenario_number, pretty_scenario_names,
...
annual_df_ranks_period_focal <- annual_statewide_df_focal_scenarios %>%
  ...
  summarise(
    ...
    # mean_cbhump = mean(costs_benefits_hump),
    # mean_cbblwh = mean(costs_benefits_blwh),
    mean_bchump = mean(benefits_costs_hump),
    mean_bcblwh = mean(benefits_costs_blwh),
    .groups = "drop"
  ) %>%
  group_by(pre_post) %>%
  mutate(
...
write_csv(annual_df_focal_scenarios_statewide_ranks, 
          here::here(
            ...
            "full_analysis_samhourietal",
            "output_dfs",
            "annual_df_focal_scenarios_statewide_ranks.csv")
          )
...
percent_diff_period_df <- annual_df_ranks_period_hump %>%
  ...
  summarise(
    ...
    percent_diff_bchump = 100 * (mean_bchump-lag(mean_bchump))/lag(mean_bchump),
    percent_diff_bcblwh = 100 * (mean_bcblwh-lag(mean_bcblwh))/lag(mean_bcblwh),
    .groups = "drop") %>%
...
save.image(paste0("/Users/jameal.samhouri/Documents/RAIMBOW/Processed Data/Samhouri et al. whales risk/Output_Data/scenario_output_dataframes/run_analysis_data_",today(),".RData"))
