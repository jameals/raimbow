...
max_speed <- 4.11556
min_speed <- 0
pre_period <- c("2009_2010","2010_2011", "2011_2012","2012_2013", "2013_2014")
mhw_period <- c("2014_2015", "2015_2016", "2016_2017", "2017_2018")
post_period <- c("2018_2019")
...
dcrb_ca_vms_tix_analysis <- dcrb_vms_tix_analysis %>%
  ...
  mutate(#GRID5KM_ID = as.character(GRID5KM_ID),
         ...
         period = case_when(
           crab_year %in% pre_period == TRUE ~ "pre",
           crab_year %in% mhw_period == TRUE ~ "mhw",
           crab_year %in% post_period == TRUE ~ "post",
           TRUE ~ "ERROR"
         ),
  ...
  dplyr::select(
    ...
    westcoastdate_notime, year, crab_year, period, year_month, month, month_as_numeric, week_of_year, day_of_year, season,
    GRID5KM_ID, BIA_mn_noNAs, BIA_bm_noNAs, BIA_bm_or_mn, pacfin_port_code, port_group_code, DEPTH_CATM, NGDC_M,
    TARGET_lbs, TARGET_rev, DCRB_lbs, DCRB_revenue
    ) # BAND_25KM, BAND_50KM, CA_OFFSHOR, Region, 
...
con_df_daily_years_5km_CA <- dcrb_ca_vms_tix_analysis_TripInfo %>%
  ...
  # ) %>%
  group_by(year, crab_year, period, year_month, season, month, month_as_numeric, week_of_year, day_of_year, GRID5KM_ID, BIA_mn_noNAs, BIA_bm_noNAs, BIA_bm_or_mn) %>% #BAND_25KM, BAND_50KM, CA_OFFSHOR, Region, 
  summarise(
...
con_df_daily_years_5km_CA_sm <- dcrb_ca_vms_tix_analysis_TripInfo %>%
  ...
  group_by(year, crab_year, period, year_month, season, month, month_as_numeric, week_of_year, day_of_year, GRID5KM_ID, BIA_mn_noNAs, BIA_bm_noNAs, BIA_bm_or_mn, Vessel_Size) %>% #BAND_25KM, BAND_50KM, CA_OFFSHOR, Region, 
  summarise(
    DCRB_lbs = sum(DCRB_lbs_per_VMSlocation),
    DCRB_rev =sum(DCRB_rev_per_VMSlocation),
    Num_DCRB_VMS_pings = n(),
...
con_df_year_month_5km_CA_sm <- dcrb_ca_vms_tix_analysis_TripInfo %>%
  filter(Vessel_Size != paste0(">=",vessel_size_category_break, " ft") ) %>%
  group_by(year, crab_year, period, year_month, season, month, month_as_numeric, GRID5KM_ID) %>% # BAND_25KM, BAND_50KM, CA_OFFSHOR, Region, BIA_mn_noNAs, BIA_bm_noNAs, BIA_bm_or_mn
  summarise(
    DCRB_lbs = sum(DCRB_lbs_per_VMSlocation),
    DCRB_rev =sum(DCRB_rev_per_VMSlocation),
    Num_DCRB_VMS_pings = n(),
