...
con_df_daily_years_5km_CA_sm <- dcrb_ca_vms_tix_analysis_TripInfo %>%
  ...
  group_by(year, crab_year, year_month, season, month, month_as_numeric, week_of_year, day_of_year, GRID5KM_ID, BIA_mn_noNAs, BIA_bm_noNAs, BIA_bm_or_mn, Vessel_Size) %>% #BAND_25KM, BAND_50KM, CA_OFFSHOR, Region, 
  summarise(
    DCRB_lbs = sum(DCRB_lbs_per_VMSlocation),
    DCRB_rev =sum(DCRB_rev_per_VMSlocation),
    Num_DCRB_VMS_pings = n(),
