...
# run make_tradeoff_dataframes_function.R first from effort_shift_comparison.R
annual_statewide_df <- read_rds("/Users/jameal.samhouri/Documents/RAIMBOW/Processed Data/Samhouri et al. whales risk/Output_Data/annual_statewide_effort_shift_scenariocomparisons_2020-05-18.rds")
glimpse(annual_statewide_df)

# make plotting year column for seasonal df's
annual_statewide_df$plotting.year <- as.numeric(substr(annual_statewide_df$crab_year,6,9))
glimpse(annual_statewide_df)
...
# annual_statewide_df_delay <- annual_statewide_df[delay_rows,]
annual_statewide_df_delay <- annual_statewide_df %>%
  filter(
...
# annual_statewide_df_early_closure <- annual_statewide_df[early_closure_rows,]
annual_statewide_df_early_closure <- annual_statewide_df %>%
  filter(
...
p_early_closure_hump <- ggplot(
  annual_statewide_df_early_closure, #df
  aes(
    x=plotting.year,
    y=risk_humpback,
