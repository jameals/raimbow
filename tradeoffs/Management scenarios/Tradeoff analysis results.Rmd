...
# PICK UP HERE TO INCLUDE ZEROES FOR EACH YEAR
...
map_hump <- ggplot() + 
  ...
  ggtitle("2009-2019 Median humpback densities on Dungeness crab fishing grounds\nduring the fishing season") +
...
map_blue <- ggplot() + 
  ...
  ggtitle("2009-2019 Median blue whale occurrence on Dungeness crab fishing grounds\nduring the fishing season") +
  coord_sf(xlim=c(sq_bbox[1],sq_bbox[3]),ylim=c(sq_bbox[2],sq_bbox[4])) + 
  theme_minimal() + #theme_classic() +
  theme(text=element_text(family="sans",size=10,color="black"),
        legend.text = element_text(size=10),
...
map_pings <- ggplot() + 
  ...
  ggtitle("2009-2019 Median VMS pings\nduring the Dungeness crab fishing season") +
  coord_sf(xlim=c(sq_bbox[1],sq_bbox[3]),ylim=c(sq_bbox[2],sq_bbox[4])) + 
  theme_minimal() + #theme_classic() +
  theme(text=element_text(family="sans",size=10,color="black"),
        legend.text = element_text(size=10),
...
## Avg risk in 2009-13 v 2014-19 for blues and humps by scenario. All vessels
...
annual_statewide_df_focal_scenarios %<>% mutate(
  ...
  pre_post = ifelse(plotting_year >=2015 & plotting_year <2019, "2014-2018",
                    ifelse(plotting_year <2015, "2009-2014", "2018-19")
  )
) 
glimpse(annual_statewide_df_focal_scenarios)
...
# normalized humpbacks, statewide only
...
# 081420. pick up here. rank order of scenarios is still wrong. make sure to make revenuee plots above.
...
# there is a more subtle point to get across. delay cenCA + statewide closure is nearly as good as statewide delay and closure. seems fine for SM.
# also can use SM to point out that BIA closures not so helpful
### all scenarios

# arrange from greatest risk during 2014-18 to least
annual_rank <- annual_statewide_df_focal_scenarios %>%
  group_by(pretty_scenario_names, pre_post) %>%
  summarise(
    mean_n_risk_blue = mean(n_risk_blue),
    mean_n_risk_humpback = mean(n_risk_humpback),
...
annual_rank_nosq <- annual_rank %>% filter(pretty_scenario_names != "Status Quo") 
annual_rank_plot <- annual_rank[which(annual_rank$pretty_scenario_names == "Status Quo" & annual_rank$pre_post == "2014-2018"),] %>%
  bind_rows(annual_rank_nosq) %>%
  bind_rows(annual_rank[which(annual_rank$pretty_scenario_names == "Status Quo" & annual_rank$pre_post == "2009-2014"),]) %>%
  bind_rows(annual_rank[which(annual_rank$pretty_scenario_names == "Status Quo" & annual_rank$pre_post == "2018-2019"),]) %>%
mutate(scenario_rank = row_number())  
glimpse(annual_rank_plot)

annual_rank_bp <- annual_statewide_df_focal_scenarios %>%
  left_join(annual_rank)
...
### below here is probably trash

statewide_annual_rank_bp <- statewide_annual %>%
  left_join(statewide_annual_rank)
glimpse(statewide_annual_rank_bp)

annual_statewide_df_focal_scenarios_means <- annual_statewide_df_focal_scenarios %>% 
...
annual_statewide_df_focal_scenarios_means_ranks <- annual_statewide_df_focal_scenarios_means %>%
  group_by(pretty_scenario_names) %>%
  filter(pre_post == "2014-2018") %>%
  mutate(
    scenario_rank_during = scenario_rank
  ) %>%
  full_join(annual_statewide_df_focal_scenarios_means)
glimpse(annual_statewide_df_focal_scenarios_means_ranks)

annual_statewide_df_focal_scenarios_means_bp <- annual_statewide_df_focal_scenarios %>%   left_join(annual_statewide_df_focal_scenarios_means) %>%
  mutate(
  ...
  group_by(pretty_scenario_names) %>%
  mutate(
    scenario_rank_min = min(scenario_rank)
  )
glimpse(annual_statewide_df_focal_scenarios_means_bp)

# move SQ scenario first

annual_statewide_df_focal_scenarios_means_ranks <- annual_statewide_df_focal_scenarios_means_ranks %>%
  filter(pretty_scenario_names != "Status Quo") %>%
  mutate(
    scenario_rank_2 = scenario_rank_during + 1
  ) %>%
  bind_rows(annual_statewide_df_focal_scenarios_means_ranks[which(annual_statewide_df_focal_scenarios_means_ranks$pretty_scenario_names == "Status Quo"),])

annual_statewide_df_focal_scenarios_means_ranks$scenario_rank_2 <-  ifelse(annual_statewide_df_focal_scenarios_means_ranks$pretty_scenario_names == "Status Quo", 1, annual_statewide_df_focal_scenarios_means_ranks$scenario_rank_2)

annual_statewide_df_focal_scenarios_means_bp <- annual_statewide_df_focal_scenarios_means_bp %>%
  filter(pretty_scenario_names != "Status Quo") %>%
  mutate(
    scenario_rank_2 = scenario_rank_min + 1
  ) %>%
  bind_rows(annual_statewide_df_focal_scenarios_means_bp[which(annual_statewide_df_focal_scenarios_means_bp$pretty_scenario_names == "Status Quo"),])

annual_statewide_df_focal_scenarios_means_bp$scenario_rank_2 <-  ifelse(annual_statewide_df_focal_scenarios_means_bp$pretty_scenario_names == "Status Quo", 1, annual_statewide_df_focal_scenarios_means_bp$scenario_rank_2)

#View(annual_statewide_df_focal_scenarios_means_bp)

# normalized humpbacks
plot_hump_risk_prepost <- ggplot(
  data = annual_statewide_df_focal_scenarios_means_ranks,
  aes(
    x = reorder(pretty_scenario_names,-scenario_rank_2),
    y = mean_n_risk_humpback,
    colour = factor(pre_post)
    )
  )  +
  ...
  width=0.1) + 
  #geom_label(aes(label = crab_year, group = factor(pre_post)), size=2, alpha=0.6, position=position_jitterdodge(jitter.height = 0.25, dodge.width=0.75)) +
    #stat_err(geom="point", size=5, alpha = 0.8) + #,
           #position=position_jitterdodge(jitter.width = 0.25, jitter.height = 0.25, dodge.width=0.75)
  #stat_err(geom = "errorbar", spread = "se", mult = 2, width=.5) + #position=position_jitterdodge()
  #coord_flip() +
...
png(paste0(path_figures, "/plot_hump_risk_prepost.png"), width = 14, height = 10, units = "in", res = 300)
plot_hump_risk_prepost
dev.off()

# normalized blues
plot_blue_risk_prepost <- ggplot(
  ...
  #geom_label(aes(label = crab_year, group = factor(pre_post)), size=2, alpha=0.3, position=position_jitterdodge()) +
  geom_point(alpha=0)+
  stat_err(spread = "se", mult = 2, width=.1) + # set mult=1 and spread="se" for 1 SE
  stat_err(geom="point", size=5, alpha = 0.8) + 
  #coord_flip() +
  # scale_y_continuous(limits = c(-26,100),
  #                    breaks = c(-25,0,25,50,75,100)) +
...
png(paste0(path_figures, "/plot_blue_risk_prepost.png"), width = 14, height = 10, units = "in", res = 300)
plot_blue_risk_prepost
dev.off()
...
## Avg $ in 2009-13 v 2014-19 for all DCRB vessels by scenario.
...
## Tradeoff plots for early and late time period separately for hump, blues, both. Full time period, 2009-13, 2014-19. All DCRB vessels.
