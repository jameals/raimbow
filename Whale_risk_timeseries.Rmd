...
This document summarizes and plots time series of humpback whale risk of entanglement, as well as the humpback predictions and fishing measure values. It loads values generated in 'Whale_risk.Rmd', and converts the humpback and risk densities to absolute values (abundance and abundance * VMS measure, respectively) in order to sum values by region. These summed values, as well as their densities, are then plotted. In addition, we plot the monthly change (x / x_mean) and z score ((x - x_mean) / x_sd), where the mean and sd are calculated by region/month and using only 2009 - 2013 values.

The whale risk files, e.g. 'Whale_risk.Rdata', is created in 'Whale_risk.Rmd' from the [raimbow-whaleRisk GitHub repo](https://github.com/smwoodman/raimbow-whaleRisk).

Outstanding questions:

* Should mean/sd be calculated using 2009-13 (full calendar year), or 2009-13 fishing seasons (excluding Nov and Dec 2013)?
* What to do when 2009-13 mean/sd is 0, but a later year has a non-0 value?
...
rm(risk.all, risk.total, humpback.abund, humpback.all, fish.all)
```
...
# if (save.flag) save(all.df, all.df.summ, 
#                     file = paste0(path.rdata, "Whale_risk_timeseries.Rdata"))
...
In addition to visualizing the actual risk, humpback, and fishing values, we want to examine the anomalies for these values, i.e. the difference of values in one year from the mean value. Specifically, we want to examine how values differ from the mean values from 'normal years' 2009 - 2013. We will evaluate these anomalies both by 1) dividing the value by the mean value and 2) using z-scores, i.e. (x - x_mean) / x_sd. 

First we calculate the mean and sd values:

```{r} 
base.mean <- all.df.summ %>% 
  filter(yr < 2014) %>% #, !((yr == 2013) & (mo %in% c(11, 12)))) %>%
  select(-ym, -yr) %>% 
  group_by(region, mon) %>% 
...
Now we calculate the anomaly ('change') value...

```{r, fig.width=10, fig.height=10}
all.df.anom <- all.df.summ %>% 
  select(region, ym, yr, mon) %>% 
  bind_cols(tmp.vals / replace(tmp.mean, tmp.mean == 0, 0.1))
  

p1 <- raimbow_ggplot(
  all.df.anom, risk_sum_dens , plot.main = "Risk - anomalies", 
  y.lab = "Monthly anomaly relative to 2009-13"
)
p2 <- raimbow_ggplot(
...
...and the z scores. Note that for both, we change 0 values in the denominator to 0.1 so as to avoid divide-by-zero errors. However, this value is arbitrary and this method could likely be improved.

```{r, fig.width=10, fig.height=10}
all.df.zscore <- all.df.summ %>% 
  select(region, ym, yr, mon) %>% 
  bind_cols((tmp.vals - tmp.mean) / replace(tmp.sd, tmp.sd == 0, 0.1)) #tmp.sd)

p1 <- raimbow_ggplot(
  all.df.zscore, risk_sum_dens , plot.main = "Risk - anomalies", 
  y.lab = "Std dev from 2009-13 mean"
)
p2 <- raimbow_ggplot(
  all.df.zscore, mn_sum_dens , plot.main = "Humpback whales - anomalies", 
  y.lab = "Std dev from 2009-13 mean"
)
p3 <- raimbow_ggplot(
  all.df.zscore, vms_sum_dens , plot.main = "Fishing - anomalies", 
  y.lab = "Std dev from 2009-13 mean"
)
