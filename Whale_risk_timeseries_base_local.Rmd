---
title: "Whale risk time series baseline - first look"
author: "Sam Woodman"
date: "1/8/2020"
...
This analysis will need to be done with grid-cell-calculated risk, perhaps once confidential data is in-hand.?
...
We can plot the baseline-whale risk density values, along with the baseline whale and non-baseline (i.e., all) fishing density values. Note that the y-axis limit of the whale plot was made to be identical to the non-baseline whale plot above. Also note that these are the total values rather than densities. However, note also that the y-axis scale between the risk panels on these two plots is quite different.

```{r, fig.height=10, fig.width=10}
p1 <- raimbow_ggplot(
  x, risk_sum_dens_basew, plot.main = "Risk - rcr - basew", 
  y.lab = "Whales (baseline) * VMS pings / km2"
)
p2 <- raimbow_ggplot(
  x, mn_sum_dens_base, plot.main = "Humpback whales (baseline)", 
  y.lab = "Whales / km2"
) + scale_y_continuous(limits = c(0, max(x$mn_sum_dens)))
p3 <- raimbow_ggplot(
  x, vms_sum_dens, plot.main = "Fishing", 
  y.lab = "VMS pings / km2"
)
...
```{r, fig.height=10, fig.width=10}
p1 <- raimbow_ggplot(
  x, risk_sum_dens_basef, plot.main = "Risk - rcr - basef", 
  y.lab = "Whales * VMS pings (baseline) / km2"
)
p2 <- raimbow_ggplot(
  x, mn_sum_dens, plot.main = "Humpback whales", 
  y.lab = "Whales / km2"
)
p3 <- raimbow_ggplot(
  x, vms_sum_dens_base, plot.main = "Fishing (baseline)", 
  y.lab = "VMS pings / km2"
) + scale_y_continuous(limits = c(0, max(x$vms_sum_dens)))
...
We can make plot 'baseline-whale' and 'baseline-fish' risk values with original risk values for direct comparison. Notes that the y-axis max is set as the same for all three panes in subsequent plots.

We have the risk as total sums for each region...

```{r, fig.height=10, fig.width=10}
p.max <- max(c(x$risk_exp_total, x$risk_sum_total_basew, x$risk_sum_total_basef))
p1 <- raimbow_ggplot(
  x, risk_exp_total, plot.main = "Risk - rcr", 
  y.lab = "Whales * VMS pings"
) + 
  scale_y_continuous(limits = c(0, p.max))

p2 <- raimbow_ggplot(
  x, risk_sum_total_basew, plot.main = "Risk - rcr - basew", 
  y.lab = "Whales (baseline) * VMS pings"
) + 
  scale_y_continuous(limits = c(0, p.max))

p3 <- raimbow_ggplot(
  x, risk_sum_total_basef, plot.main = "Risk - rcr - basef", 
...
```{r, fig.height=10, fig.width=10}
p.max <- max(c(x$risk_exp_dens, x$risk_sum_dens_basew, x$risk_sum_dens_basef))
p1 <- raimbow_ggplot(
  x, risk_exp_dens, plot.main = "Risk - rcr", 
  y.lab = "Whales * VMS pings / km2"
) + 
  scale_y_continuous(limits = c(0, p.max))
